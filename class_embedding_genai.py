...
class PatentClassifier:
    ...
    def preprocess_data(self):
        """
        Prepare data for text classification and vectorization using embeddings.

        This function extracts the necessary information from the combined dataset, calculates the distribution
        of labels, and transforms the text data into numerical vectors using spaCy's word embeddings.
        """
        ...
        try:
            ...
            import spacy.cli
            spacy.cli.download("en_core_web_lg")  # Downloads the model if it's not already installed.
            self.nlp = spacy.load('en_core_web_lg')  # Loads the model after downloading.

        # Convert each abstract into vector representation
        self.text_vectors = np.array([self.nlp(doc).vector for doc in tqdm(self.texts, desc='Computing embeddings')])  # Uses spaCy to convert each patent abstract into a numerical vector based on word embeddings.

        # Output the dimension of the embeddings
        print(f"Embedding dimension: {self.text_vectors.shape[1]}")  # Prints the size of each embedding vector.
    ...
    def train_and_evaluate(self):
        ...
        CLASSIFIERS = [
            ["Support Vector Classifier (RBF)", SVC(probability=True)],  # SVC with RBF kernel and probability estimates.
            ["Logistic Regression", LogisticRegression(max_iter=1000)],  # Logistic Regression classifier with increased iterations for convergence.
        ...
        for CL in tqdm(CLASSIFIERS, desc="Evaluating Classifiers"):
            ...
            for train_i, test_i in tqdm(KFoldSplitter.split(Abstract_Vectors, Y),
                ...
                temp_X = list(train_X)  # Create a temporary list of training feature vectors.

                if Reweight:
                    # Repeat balancing to achieve approximately equal class proportions
                    for _ in range(3):  # Repeat the balancing process three times.
                        for i in range(len(train_y)):  # Iterate over each training example.
                ...
                # Convert temp_X to numpy array for model training
                ...
                if hasattr(Model, "predict_proba"):  # Checks if the model can predict probabilities.
                    y_pred_proba = Model.predict_proba(test_X)[:, 1]  # Predicts probabilities for the positive class (GenAI).
                    y_pred = [1 if y > 0.5 else 0 for y in y_pred_proba]  # Converts probabilities to binary predictions.
                else:
                    y_pred = Model.predict(test_X)  # Predicts labels directly.
                ...
                if name == "Random Forest":
                    y_pred_p = Model.predict_proba(test_X)[:, 1]  # Predicts probabilities for the positive class (GenAI).
                    y_predicted_p += list(y_pred_p)  # Appends predicted probabilities.
        ...
        self.results_table["Type"] = "Embeddings"  # Specifies that word embeddings were used for feature extraction.
        ...
        # Sort the table by accuracy and save the results as CSV
        self.results_table.sort_values("Accuracy", ascending=False).to_csv(
            f"./Output/GenAI_Embeddings_Model_Classification_Performance_{self.total_entries}_{self.total_ratio}.csv",
        ...
        # Create a DataFrame for each classification and merge them
        ...
        Final.to_csv(f"./Output/GenAI_Embeddings_Classification_Results_{self.total_entries}_{self.total_ratio}.csv", index=False)  # Saves the detailed classification results.
        ...
        for i, classified_values_p in enumerate(self.classified_values_p):
            ...
            if i == 0:
                ...
                Temp = Temp[['id', 'Actual', 'Predicted']]  # Selects relevant columns.
                Temp.columns = ['id', 'Actual', name]  # Renames columns to include the model name.
                Final_p = Temp  # Initializes the Final_p DataFrame with the first model's probability results.
            else:
                name = Temp['Model'].iloc[0]  # Retrieves the model name from the first row.
                Temp = Temp[['id', 'Predicted']]  # Selects relevant columns.
                Temp.columns = ['id', name]  # Renames columns to include the model name.
                Final_p = Final_p.merge(Temp, on='id')  # Merges with the Final_p DataFrame based on 'id'.

        # Save the probabilities as a CSV file
        Final_p.to_csv(f"./Output/GenAI_RandomForest_Embeddings_Predicted_Probabilities_{self.total_entries}_{self.total_ratio}.csv", index=False)  # Saves the predicted probabilities for the Random Forest model.
    ...
    def plot_all_metrics(self):
        ...
        ax.set_title(
            f'Comparison Embeddings-Model Performance ({self.total_entries} patents, ratio: {self.total_ratio})')  # Sets the title of the chart.
        ...
        plt.savefig(f"Figures/emb_model_performance_grouped_bar_{self.total_entries}_{self.total_ratio}.png", dpi=300)  # Saves the figure as a high-resolution PNG file.
...
# python class_embedding_genai.py
...
if __name__ == '__main__':
    ...
    # Preprocess data for classification and vectorization using embeddings
